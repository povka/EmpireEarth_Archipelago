...
being buildable when you advance, so they're left out.
...
* `UNIT_FAMILY_BY_NAME` maps every unit to its family. Checks are per unit
  ...
  (`Producers.py`) and which epoch it belongs to, so the grouping hasn't gone
  away — only the checks built on it have.
* `UNIT_FAMILIES` and `UNIT_FAMILY_MIN_EPOCH` are that grouping.

Reads the Art of Conquest database by default, because that's the edition the
...
and a building with no measured epoch isn't offered as a check at all. Unit
epochs get corrected here instead; see `unit_epoch`.
"""
...
FAMILY_OFF = 0x68
# Category, and the earliest epoch the object can be built in. There's no
# matching "last epoch" field — things like Guard Tower - Paleo stop being
# buildable because a later variant supersedes them by name, not by data.
CATEGORY_OFF = 0x6C
MIN_EPOCH_OFF = 0x70
WONDER_CATEGORY = 28

# Wonders take the same one-indexed correction as everything else, plus a floor
# on top. Leaving the raw number in place put `Orbital Space Station` at epoch
# 15 — one past the Space Age — so no seed could offer it, and it got reported
# built in a Space Age match.
#
# The floor is the part that isn't an off-by-one. It reads 1 (Stone Age) for
# six of the seven, but you can't actually build a wonder until the Bronze Age.
# Established in game, not guessed:
#
...
# every early wonder turns up only the family id, which is 3 for "Building" by
# coincidence — so the rule lives somewhere else in the engine and this goes on
# top of the database value.
...
# Buildings whose name carries an epoch, or that are otherwise a variant.
# Excluded, because they stop being buildable when you advance.
BUILDING_EXCLUDE_MARKERS = (
    ...
    "iwo jima",
    # The Farm is kept out by choice, not because it's a variant. It's the
    # one building with no tech tree node the client can reach — the only icon
    # mentioning a farm is `but_farm_15t`, an epoch 14 variant — so it can't be
    # gated and its epoch can't be confirmed against the running game the way
    # every other building's is. That leaves its floor coming from
...
# family". Now that every unit has its own check, anything left out has no
# check at all — which is how `Inf01 - Rock Thrower` came to be unsendable. Its
# family is `Human`, and `Human` wasn't on the list. Nor were `Hero`,
...
# error here — it tells logic that a building you can't use is good enough, and
# `Producers.py` drops ambiguous labels for the same reason.
#
# It costs eight checks: three spaceships, three space fighters, a space
# corvette and a campaign ICBM. To admit them, run
# `tools/gen_building_epochs.py` against a live Art of Conquest match and give
# each family a producer.
...
# nothing can recruit one and a check for it could never be sent. `Hurricane`
# and `Torpedo` are filed under Ship, `Anti Matter Storm` under Helicopter, all
# three at epoch 0 with 9999 hitpoints.
#
# The family floor already ignored them, because it skips epoch 0. The per-unit
# tables didn't, so they arrived in Objects.py as recruitable units. Excluded
# here rather than deleted from the generated file by hand — this is the only
# place the exclusion survives a regeneration.
NON_UNIT_NAMES = {
    "Hurricane",
    "Torpedo",
    "Anti Matter Storm",
...
def name_tier(name: str) -> int:
    ...
    as nothing at all. Both are wrong and neither matters, because this only
    ever raises a floor; see `unit_epoch`.
    """
...
def unit_epoch(raw: int, name: str = "") -> int:
    ...
    database share a numbering, and it's one above the tree's. For units the
    ...
    printed VIII and stored 9) — never one low, so subtracting one can only
    over-state an epoch, which costs a check but can't make a seed unwinnable.

    Leaving the +1 in place is what made `Recruit Cataphract` a Middle Ages
    check when a Cataphract is a Dark Age unit.

    The name's own tier is taken when it's *higher*, and only then. Three
    records Art of Conquest adds store an epoch one below the tier they're named
    for, and that's the dangerous direction — a floor that reads low lets
    generation hide an epoch item behind a check that needs it:
    ...
    Taking the higher of the two can only over-state an epoch, which is the
    side that costs a check rather than a run. It's also why the tier parse is
    allowed to be sloppy — every name it misreads, it misreads low.
    """
    return max(raw, name_tier(name)) - 1 if max(raw, name_tier(name)) else 0


# Where the database's name isn't the game's. `w  Lighthouse at Alexandria` is
# the Pharos Lighthouse — a different wonder from the Library of Alexandria,
# which the database also carries — and the display name is what you read in
...
def emit(buildings, unit_families, members, wonder_list) -> str:
    ...
    lines = [
        ...
        "",
        "Don't edit by hand — re-run the generator against db\\\\dbobjects.dat.",
        "",
    ...
    lines += [
        ...
        "# something you can't build yet has to require the epochs that unlock",
    ...
    lines += [
        ...
        "# Wonders have no per-epoch variants, so none of them ever expires.",
        "# The epoch is a floor, and it's why a wonder only gets an unlock item",
        "# when the seed's goal epoch reaches it.",
