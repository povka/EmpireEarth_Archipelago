...
texture, which is the only handle the client has for finding a technology's
node in memory - and the node is where research is detected, at `+0x04`.

So this has to run once with Empire Earth open, in a match. After that the
generated module is committed and generation never needs the game again.
...
of where the technology belongs. Names that differ too much to trust fuzzy
matching are listed in ALIASES below rather than guessed at.
"""
import difflib
import io
import os
...
# fuzzy matching cannot be trusted to get it right.
ALIASES = {
    "Hafted Tools": "hafts",
    "Iron Saw": "saw",
...
def stem(tex):
    ...
    # Some technologies carry a tier-style suffix too: the Space Age
    # Granary tech is `but_farm_15t`, which is why the Farm building
    # appeared to have a node when it has none.
...
def main():
    ...
    for base, data in p.snapshot(want_image=False, want_private=True,
        ...
        while True:
            ...
            if tex:
                ...
                # state object. It is static game data - (epoch+1)*1000 plus an
    ...
    lines = [
        ...
        "",
        "Generated by tools/gen_technologies.py; do not edit by hand. The names,",
        ...
        "#",
        "# The node epoch is carried because a texture is not unique: every wall",
        "# and tower upgrade shares `but_upgrade wall and tower`, and only the",
        "# epoch on the node tells the seven of them apart. Keying lookups on the",
    ...
    lines += [
        ...
        "}",
        "",
        "# (texture, node epoch) -> name. That pair is what identifies a node;",
        "# the texture alone is not unique.",
