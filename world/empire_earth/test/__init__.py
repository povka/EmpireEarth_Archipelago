...
are not merely untidy, they never run. Everything below used to live here, and
had gone stale enough to fail on import without anything noticing.

These need an Archipelago source checkout (`python -m pytest worlds/empire_earth`);
a frozen install ships no test framework. `tools/test_generation.py` covers the
same ground against a frozen install.
"""
